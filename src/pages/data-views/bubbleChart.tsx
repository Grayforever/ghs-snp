...
const BubbleChart: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const simulation = d3.forceSimulation(root.leaves())
            ...
            .force('collision', d3.forceCollide().radius(d => d.r + 2).strength(0.2));

        const svg = d3.select(chartRef.current)
            .attr('viewBox', [0, 0, width, height])
            .attr('text-anchor', 'middle');

        const leaf = svg.selectAll<SVGGElement, BubbleData>('g')
            .data(root.leaves())
            .join('g')
        ...
        leaf.append('text')
            ...
            .join('tspan')
            .attr('x', 0)
            .attr('y', (_, i, nodes) => `${i - nodes.length / 2 + 1.6 + subtitleMargin}em`)
            .text(d => d)
        ...
        <Stack direction={"row"} spacing={2} alignItems={"center"}>
            {data.map((item, index) => (
                <div key={index} className={classes.colorLegendItem}>
                    ...
                    />
                    <Typography variant='caption' fontWeight={"bold"}>{item.position}</Typography>
                </div>
            ))}
        </Stack>
    </Stack>;
