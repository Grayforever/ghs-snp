...
export const BlogBanner = () => (<Stack spacing={2} sx={{ width: "100%" }}>
    ...
    <Stack direction={"row"} spacing={1} sx={{ width: "100%" }} justifyContent={"space-around"}>
        {["All", "Blog", "More", "Other"].map((label) => (<Button key={label}>{label}</Button>))}
    </Stack>
</Stack>)
    ...
    return <Box pt={24} pb={12}>
        <Stack mx={4}>
            ...
            <Grid container spacing={3}>
                {data.map((card) => (<Grid item key={card.id} md={6} xs={12}><BlogCard {...card} /></Grid>))}
            </Grid>
        </Stack>
